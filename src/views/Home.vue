...
<template>
    <div class=" fixed overflow-scroll sm:left-15 lg:ml-64 mt-12 h-screen pb-24 w-screen p-5">
        ...
        <div class="sm:px-12 ">
            <div class=" sm:w-1/3 md:ml-80 text-center">
                <p>{{ dayName }}</p>
                <p class="text-4xl">{{ welcomeMessage }}</p>

            </div>
            <div class="sm:w-1/2  md:ml-52 ">
                <h3 class="p-2 border-b font-semibold mb-2">
                    <i class="fa-solid fa-caret-right mr-4"></i> Recent Projects
                </h3>
                <div class="flex-wrap flex ">
                    <router-link v-for="project in projects" :key="project._id"
                        class="text-center p-1  cursor-pointer  w-1/4" :to='`/board/${project._id}`'>
